...
@section('title', 'Règlements & Factures')
...
<div class="animate-fade-in-up">
    <div class="page-header mb-6">
        <div>
            ...
            <p style="color: #64748b; font-size: 0.875rem; margin-top: 0.25rem;">Suivez l'état de vos factures et les paiements de vos clients.</p>
        </div>
    </div>
    ...
    <div class="card mb-6 p-4">
        <div class="flex gap-2 overflow-x-auto pb-2">
            ...
            <a href="{{ route('fournisseur.factures.index', ['status' => 'payee']) }}" 
                    ...
                    @forelse ($factures as $facture)
                        <tr class="hover:bg-slate-50 transition-colors">
                            <td class="py-3 px-4">
                                ...
                                <p class="text-xs text-slate-500 mt-1">Réf Cmd: <a href="{{ route('fournisseur.commandes.show', $facture->commande) }}" class="hover:underline">{{ $facture->commande->reference }}</a></p>
                            </td>
                            <td class="py-3 px-4">
                                <span class="font-medium text-slate-800">{{ $facture->commande->pharmacien->name }}</span>
                            </td>
                            ...
                            <td class="py-3 px-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('pdf.facture', $facture) }}" class="text-teal-600 hover:text-teal-800 p-1" title="Télécharger PDF">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                                    </a>
                                    
                                    @if($facture->status !== 'payee')
                                        <form method="POST" action="{{ route('fournisseur.factures.payee', $facture) }}" onsubmit="return confirm('Confirmer la réception du paiement ?');">
                                            @csrf
                                            <button type="submit" class="btn btn-sm btn-outline-primary" style="padding: 0.25rem 0.5rem; font-size: 0.75rem;">
                                                Marquer Payée
                                            </button>
                                        </form>
                                    @endif
